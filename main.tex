...
Beyond summarizing the literature, the survey contributes a propagation-oriented conceptual framework, a cross-domain taxonomy with comparative mechanism syntheses, an anti-pattern and blueprint view that connects recurring failures to concrete runtime contracts, and a disturbance-oriented evaluation agenda for future systems. We argue that the next generation of stateful infrastructure---especially for intelligent services on heterogeneous hardware---should treat state management not as a storage detail but as the central systems abstraction that links observability, control, recovery, and long-horizon service quality. In particular, we show why contracts that separate local freshness versus exposure freshness in distributed retrieval and nominal retention budget versus effective retention budget in retention stores are becoming first-class systems requirements rather than implementation details.
\end{abstract}
...
The survey makes four concrete contributions. First, it offers a unified vocabulary that connects streaming, approximate execution, serving, retrieval, and memory-centric intelligent services. Second, it proposes a taxonomy and a set of comparative syntheses that separate state problems by \emph{access}, \emph{execution}, and \emph{evolution} while emphasizing their feedback loops. Third, it distills recurring anti-patterns and converts them into a blueprint-oriented view of runtime contracts, control surfaces, and disturbance-aware evaluation, including explicit contracts for shard-exposure freshness and budget-elastic retention. Fourth, it identifies a research agenda for integrated runtimes that coordinate state observability, scheduling, update control, recovery semantics, and reusable memory services on heterogeneous hardware.
...
This section deepens the survey with domain-specific comparative syntheses. Each subsection answers the same three questions: which state object dominates in the domain, which runtime controls are mature, and which control seams remain under-developed.
...
Recent follow-on systems sharpen that fourth seam by moving from tenant-local multiplexing to explicit cross-model memory governance. S-LoRA shows that once thousands of LoRA adapters share a base model, adapter weights and KV tensors must be managed through one paging and batching surface rather than through separate memory paths, because adapter fetch overhead and heterogeneous batch composition otherwise erase the gains of parameter-efficient multiplexing~\cite{sheng2023slora}. Jenga exposes a complementary form of heterogeneity inside the serving state plane itself: when models or layers have different embedding and cache shapes, allocator structure and layer-aware eviction policy directly determine whether reusable KV state remains compact enough to sustain batching~\cite{zhang2025jenga}. Prism pushes the same logic to the multi-model regime by treating GPU KV memory, model residency, and reactivation latency as jointly governed state objects; dynamic remapping, eviction, and SLO-aware scheduling matter because long-tail demand and bursty arrivals otherwise strand memory in idle models while active models miss their latency targets~\cite{yu2025prism}. Read together with Punica and AlpaServe, these systems suggest that the emerging bottleneck is not simply how to batch more requests, but how to define transferable ownership and reclaimable residency across adapters, heterogeneous cache layouts, and entire model instances under one serving control loop.
...
Practitioner-facing discussions around RAG and vector databases point to the same conclusion. Although such posts are not substitutes for controlled evaluation, they repeatedly foreground incremental index updates, freshness loss under document churn, SSD-backed storage hierarchy, vector-index tuning, and cache-style memory reuse. That convergence is useful as an operational signal: retrieval memory is already being experienced in practice as a governed lifecycle rather than as a one-time embedding artifact.
...
Milvus/Manu/\newline FreshDiskANN/IP-DiskANN \\
...
Spanner/Calvin/FaRM line \\
\cite{corbett2012spanner,thomson2012calvin,dragojevic2014farm} & distributed transaction state and replication metadata & consistency/placement/concurrency control & correctness + latency + scale & compare with serving memory ownership models \\
...
Traditional schedulers place tasks. Stateful runtimes increasingly need to schedule around state objects themselves: windows, indexes, dictionaries, KV caches, adapter pools, model-residency slots, coresets, retriever memories, and update traces. This suggests a shift toward state-centric scheduling abstractions where the runtime models who will touch which state, on what topology, under what service objectives. Recent multi-model serving systems make this concrete: S-LoRA, Jenga, and Prism show that once adapters, heterogeneous cache layouts, and whole-model activation windows contend for the same GPU pool, scheduling must reason over residency and ownership directly rather than treat memory pressure as a secondary byproduct of request placement~\cite{sheng2023slora,zhang2025jenga,yu2025prism}.
...
Figure~\ref{fig:blueprint-flow} makes the blueprint explicit as a contract flow rather than a component checklist. The point is not merely that systems need observation, decision, and actuation, but that each stage must expose the contract that the next stage relies on: typed state objects, delayed telemetry semantics, precedence-aware decision logic, idempotent actuation, and recovery-time guardrails.
...
\textbf{Expected lessons.} Improvements come less from one faster kernel and more from coherent lifecycle control of short-lived state~\cite{kwon2023pagedattention,agrawal2024sarathi,holmes2024fastgen}. Policies that optimize only allocation or only scheduling tend to plateau early. From the anti-pattern perspective, this case primarily tests whether telemetry triggers real actuation and whether admission, phase control, and reclamation obey one precedence order instead of competing heuristics.
...
\textbf{Expected lessons.} Treating elasticity and fault handling as shared state-transfer problems yields better stability than implementing them as separate subsystems~\cite{nasir2019megaphone,fernandez2013osm,zhao2024recovery}. This is the clearest test of the ownership anti-pattern: a runtime either preserves explicit transfer contracts across scale-out and replay, or it rediscovers the same instability under a different trigger.
...
\textbf{State objects.} vector index shards, late-interaction caches, graph memory, shard exposure metadata, retention metadata.

\textbf{Control surfaces.} incremental indexing policy, refresh cadence, compaction schedule, shard exposure policy, stale-memory eviction.
...
\textbf{Boundary model.} Hard constraints: quality floor, shard-exposure safety, and uptime. Soft objective: reduce amortized update cost.

\textbf{Expected lessons.} Retrieval quality and system cost should be evaluated jointly across long horizons; short static evaluations understate lifecycle debt~\cite{karpukhin2020dpr,khattab2020colbert,wang2024hipporag,zhang2026flowrag}. In anti-pattern terms, this case checks whether the design can expose write amplification and maintenance debt early enough to trigger refresh, compaction, delayed shard exposure, or stale-memory eviction before quality collapse appears at the service boundary.
...
The design implications above can be distilled into four forward-looking priorities for stateful infrastructure.
...
This survey argued that state management is the unifying systems problem behind a wide range of modern runtimes. Whether the workload is transactional streaming, hardware-conscious analytics, LLM serving, retrieval-augmented inference, or continual retention, the core challenges recur: making shared state observable, controlling access under concurrency, mapping execution onto complex hardware and service boundaries, and evolving memory without destabilizing future reuse.

The literature already provides strong ingredients for this agenda. Work on contention-aware access and scheduling established the observation-model-control chain for shared state. Hardware-conscious and quality-aware execution studies clarified why stable gains require joint modeling of topology, movement, cost, and quality boundaries. Recent work on online updates, continual retention, and dynamic retrieval showed that state evolution and reuse must be treated as first-class systems mechanisms rather than application-specific afterthoughts. The comparative syntheses in this survey suggest that these are no longer separate stories: short-lived serving memory, retrieval indexes, replay buffers, compressed operator state, and recovery metadata all behave like runtime-governed state planes with explicit lifecycles.

The practical consequence is that future systems need more than better local heuristics. They need explicit contracts for what state is visible, what boundaries are protected, which actions are legal under disturbance, how overlapping policies compose, and what recovery-time invariants survive reconfiguration. For modern retrieval and retention stacks, this includes concrete semantics for when partially refreshed shards may be exposed to user traffic and how effective retention budget is computed under compaction, demotion, and migration. That is why the anti-pattern taxonomy, blueprint contracts, and disturbance-oriented evaluation protocol belong in the same argument: together they turn state management from a descriptive label into an implementable control discipline.

The key remaining step is integration. Future systems should close the loop among access, execution, evolution, and recovery, turning state management into the central control plane of long-running services. The strongest open systems gap is therefore not one more allocator, scheduler, or retriever update rule in isolation, but a shared runtime substrate that can expose state catalogs, enforce precedence semantics, account for maintenance debt, and keep service-level guarantees stable while memory objects change lifetime, representation, and ownership over time. The integration bar should be judged by contract-level outcomes: whether runtimes can keep exposure freshness auditable across shards, keep retention budgets interpretable under elasticity, and preserve service boundaries while maintenance proceeds continuously in the background.
