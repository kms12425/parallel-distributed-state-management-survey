...
Dense dual-encoder systems improved retrieval quality and throughput tradeoffs, while late-interaction approaches preserved richer matching signals with manageable online cost by shifting computation between offline and online phases. Vector-index mechanisms such as PQ and HNSW made the same tradeoff operational by exposing compression density, graph traversal, insertion cost, and compaction behavior as concrete control surfaces over reusable retrieval state~\cite{jegou2011pq,malkov2020hnsw}. Graph- and structure-enhanced memory approaches pushed the domain toward long-horizon memory models that better capture multi-hop and evolving knowledge dependencies~\cite{wang2024hipporag,sarthi2024raptor}.

For systems researchers, the central insight is that retrieval quality is a lifecycle property, not just a model property. Refresh cadence, index maintenance policy, stale-memory eviction, and continuous-ANNS maintenance under churn govern long-term quality-cost balance as strongly as encoder architecture~\cite{wang2026candor,zhang2026flowrag}.

Open gaps include unified treatment of dense, graph, and late-interaction memory objects in one control plane, stronger operational metrics that connect index maintenance decisions to end-to-end service SLOs, and explicit policies for rebuild cadence and maintenance-debt repayment under dynamic corpora.
